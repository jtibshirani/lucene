...
public final class Lucene90HnswVectorsWriter extends KnnVectorsWriter {
  ...
  public void writeField(FieldInfo fieldInfo, KnnVectorsReader knnVectorsReader)
    ...
    int[] docIds = writeVectorData(vectors, vectorData);
  ...
  public void mergeField(FieldInfo fieldInfo, MergeState mergeState) throws IOException {
    ...
    int[] docIds = writeVectorData(vectors, tempVectorData);
    tempVectorData.close();

    // copy the vectors to the actual vector data file
    IndexInput vectorDataInput =
        segmentWriteState.directory.openInput(tempVectorData.getName(), segmentWriteState.context);
    long vectorDataLength = vectorDataInput.length();
  ...
  private static int[] writeVectorData(VectorValues vectors, IndexOutput output)
      throws IOException {
